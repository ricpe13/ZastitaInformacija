...
public class FileReceiver
{
    ...
    public FileReceiver(int port, string saveFolder) //pamti port i folder gde prima fajlove
    {
        this.port = port;
    ...
    private void ListenLoopGui(CancellationToken token) //glavna petlja za slusanje
    {
        try
        {
    ...
    private void HandleIncomingFileGui(NetworkStream stream) //prima i validira fajl
    {
        using var reader = new BinaryReader(stream, Encoding.UTF8, leaveOpen: true);

        //////citanje hesa iz zaglavlja
        ...
        while (remaining > 0)
        {
            ...
            sha.TransformBlock(buffer, 0, read, null, 0); //racuna se hes
            remaining -= read;
        }
        ...
        bool ok = ConstantTimeEquals(actualHash, expectedHash); //poredjenje hesa
        if (!ok)
        {
            fs.Close();
