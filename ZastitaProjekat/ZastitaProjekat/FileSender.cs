...
public class FileSender
{
    ...
    public bool TrySend(string filePath, string algorithm, byte[]? key, byte[]? nonce, out string? error) //tu se racuna hes nad sifratom
    {
        error = null;
        ...
        try
        {
            ...
            byte[] hash = SHA2Helper.ComputeSHA256(payloadData); //////// racuna hes

            using TcpClient client = new TcpClient();
            var connectTask = client.ConnectAsync(ip, port); //uspostavljanje veze
            ...
            ////upis hesa u zaglavlje
            writer.Write(fileNameToSend);
            writer.Write(payloadData.LongLength); //velicina ovoga sto je sifrovano
            writer.Write(hash.Length); 
            writer.Write(hash);
            writer.Write(payloadData); //sifrovani sadrzaj fajla
            writer.Flush();
